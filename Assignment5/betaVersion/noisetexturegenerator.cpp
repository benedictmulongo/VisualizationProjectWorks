...
NoiseTextureGenerator::NoiseTextureGenerator()
    ...
    , propGrayOrWB("grayOrWB", "Gray or WhiteBlack", 0, 0,2,1)
	, rndTexture_x("rndXpixels", "Pixels in x-direction", 1, 1, 1000, 1)
	, rndTexture_y("rndYpixels", "Pixels in y-direction", 1, 1, 1000, 1)
    , propSeedRandom("theSeed", "Random seed", 371, 0, RAND_MAX, 10)
    , propUseSeed("userSeed", "Users seed", 0, 0,2,1)
// TODO: Register additional properties ("useSeed", "User's seed")
{
    // Register ports
    addPort(texOut_);
    ...
    addProperty(propGrayOrWB);
	addProperty(rndTexture_x);
	addProperty(rndTexture_y);
    addProperty(propSeedRandom);
    addProperty(propUseSeed);
...
void NoiseTextureGenerator::process() {
    ...
    {
        srand(propSeedRandom);
    } 


    for (int j = 0; j < rndTexture_y; j++) 
    {
        for (int i = 0; i < rndTexture_x; i++) 
        {
            ...
            if(propGrayOrWB == 1)
            {
                ...
                val = (val > 177) ? 0 : 255;
                int pixelss = (int) val;
                pixelss = (pixelss > 255) ? 255 : pixelss;
                pixelss = (pixelss < 0) ? 0 : pixelss;
                
                noiseTexture.setPixelGrayScale(size2_t(i, j), pixelss);
            }
            else if(propGrayOrWB == 0)
            {
                val = ((double)rand() / RAND_MAX) * 255 ;
            ...
            else
            {
                ...
                float G = ((double)rand() / RAND_MAX) * 177;
                float B = ((double)rand() / RAND_MAX) * 255;

                noiseTexture.setPixel(size2_t(i, j), vec4(R,G,B, 255));
