...
StreamlineIntegrator::StreamlineIntegrator()
    ...
    , propNdistPoint("NdistPoint", "N. Dist. Points", 5, 1, 200,1)
    ...
    , propMagnVectorField("MagnitudeFields", "Magn. Fields")
...

{
    ...
    addProperty(propMagnVectorField);
...
void StreamlineIntegrator::process() {
   ...
   // const double minValue = grid.getMinValue();
    //const double maxValue = grid.getMaxValue();
        ...
        if(propMagnVectorField)
        {
            int propN = propNdistPoint;
            int n = int(sqrt(propN));
            vector<double> xAxis = linspace(min_x, max_x, n);
            vector<double> yAxis = linspace(max_y,min_y, n);
            //vector<double> yAxis = linspace(min_y, max_y, n);
            //const dvec2 bBoxMin = vectorField.getMinValue();
            //const dvec2 bBoxMax = vectorField.getMaxValue();
            // dvec2 randomPoint = generateRandom(bBoxMin, bBoxMax);
            //int seed = 1999;
            //srand(seed);
            //const dvec2 vMin = vectorField.getMinValue();
            //const dvec2 vMax = vectorField.getMaxValue();
            const dvec2 vvMin = vectorField.getBBoxMin();
            const dvec2 vvMax = vectorField.getBBoxMax();

            dvec2 vMin = vectorField.interpolate(vvMin);
            dvec2 vMax = vectorField.interpolate(vvMax);

            double vector_min = sqrt(vMin[0]*vMin[0] + vMin[1]* vMin[1] );
            double vector_max = sqrt(vMax[0]*vMax[0] + vMax[1]* vMax[1] );

            vector<double> magnitudeInCell;
            ...
            for(int i = 0; i < xAxis.size() - 1; i++)
            {
                ...
                { 
                    ...
                    {
                        //LogProcessorInfo(" vector_min -> " << vector_min << " , vector_max -> " << vector_max ); 
                        double numb_x = ((double)rand() / RAND_MAX) * (xAxis[i+1] - xAxis[i]) + xAxis[i];
                        double numb_y = ((double)rand() / RAND_MAX) * (yAxis[j] - yAxis[j+1]) + yAxis[j+1];  
                        //LogProcessorInfo(" numb_x -> " << numb_x << " , numb_y -> " << numb_y ); 
                        dvec2 rndPoint = dvec2(numb_x, numb_y);
                        double normer = velocity(vectorField, rndPoint);
                        //LogProcessorInfo(" normer -> " << normer << " , rndPoint -> " << rndPoint ); 
                        //int from =  Sign[i][j + 1];
                        //int to = Sign[i + 1][j];
                        //point_magn += (normer - vector_min) / (vector_max - vector_min) ;
                        point_magn += normer;
                        //LogProcessorInfo(" point_magn -> " << point_magn );
                    }
                    point_magn = point_magn / 100 ;
                    magnitudeInCell.push_back(point_magn);

                    count = count + 1;
                }
            }
            double sum_magn =0 ;
            for(int k = 0; k < magnitudeInCell.size(); k++)
            {
                sum_magn = sum_magn +  magnitudeInCell[k] ;
            }
            for(int k = 0; k < magnitudeInCell.size(); k++)
            {
                magnitudeInCell[k] = magnitudeInCell[k] / sum_magn ;
            }
            
            for(int k = 0; k < magnitudeInCell.size(); k++)
            {
                LogProcessorInfo(" index -> " << k << " , vector (Rnd)-> " << magnitudeInCell[k] );
            }
            ...
            for(int i = 0; i < xAxis.size() - 1; i++)
            {
                for(int j = 0; j < yAxis.size() - 1; j++)
                { 
                    double temp = propNdistPoint * magnitudeInCell[counter];
                    LogProcessorInfo(" propNdistPoint -> " << propNdistPoint << " magnitudeInCell-> " << magnitudeInCell[counter]);
                    LogProcessorInfo(" temp -> " << temp );
                    int n_points = int(temp);
                    LogProcessorInfo(" n_points -> " << n_points );
                    for(int numb = 0 ; numb < n_points; numb++)
                    {
                        //LogProcessorInfo(" vector_min -> " << vector_min << " , vector_max -> " << vector_max ); 
                        double numb_x = ((double)rand() / RAND_MAX) * (xAxis[i+1] - xAxis[i]) + xAxis[i];
                        double numb_y = ((double)rand() / RAND_MAX) * (yAxis[j] - yAxis[j+1]) + yAxis[j+1];  
                        //LogProcessorInfo(" numb_x -> " << numb_x << " , numb_y -> " << numb_y ); 
                        dvec2 rndPoint = dvec2(numb_x, numb_y);
                        LogProcessorInfo(" index -> " << numb << " , vector (Rnd)-> " << rndPoint );
                        drawStreamline(mesh,vectorField,vertices,rndPoint);
